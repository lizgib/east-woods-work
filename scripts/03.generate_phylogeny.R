...
if(!exists('tr.zanne')) tr.zanne <- read.tree('data/Phylogeny/phylo.zanne.tre')
...
write.csv(tre.ew$matrix.renaming, 'data/Phylogeny/tree_taxa.csv')
tre.ew2 <- weldTaxa(tre.ew)
tre.ew2.pruned <- drop.tip(tre.ew2, which(!tre.ew2$tip.label %in% row.names(tre.ew$matrix.renaming)))
write.tree(tre.ew2.pruned, 'data/Phylogeny/tr.ew.Spring19', tree.names = T)


tiff('data/Phylogeny/tr.ew.Spring19.tiff', units = 'in', height = 10, width = 12, res = 400)
